-- Add a concise, non-PII sample row to date-validation errors so integration
-- mapping problems can be diagnosed from Make's execution log.
...
begin
  ...
  if v_record_kind in ('ad_group', 'ad_set') then
    ...
    return public.upsert_dashboard_paid_media_group_batch(
      v_client_slug, v_source, v_group_kind, v_idempotency_key,
      v_range_start, v_range_end, v_rows
    );
  end if;

  return public.upsert_dashboard_paid_media_batch(
    v_client_slug, v_source, v_idempotency_key, v_range_start, v_range_end,
